...
public class PluginXmlRenderer extends AbstractXmlProvider implements IPluginXmlRenderer{
	...
	/*<perspective class="org.eclipse.riena.navigation.ui.swt.views.SubApplicationView" id="org.eclipse.riena.toolbox.rienademo.mainSubapp" name="nameMain"/> */
	...
	public boolean registerPerspective(BundleNode bundle, RCPPerspective perspective) {
		...
		try {
			NodeList nlViewExtensions = (NodeList) xpath.evaluate(String.format("//%s[@%s='%s']", ELEM_EXTENSION, ELEM_POINT, VALUE_EXT_POINT_PERSPECTIVES), currentDoc, XPathConstants.NODESET); //$NON-NLS-1$
	...
	public boolean registerView(BundleNode bundle, RCPView view) {
		...
		XPath xpath = XPathFactory.newInstance().newXPath();
		try {
			NodeList nlViewExtensions = (NodeList) xpath.evaluate(String.format("//%s[@%s='%s']", ELEM_EXTENSION, ELEM_POINT, VALUE_EXT_POINT_VIEWS), currentDoc, XPathConstants.NODESET); //$NON-NLS-1$
	...
	private boolean viewAlreadyExists(Document currentDoc, String viewId) throws XPathExpressionException{
		XPath xpath = XPathFactory.newInstance().newXPath();
		// look for an element view with an ID = viewId anywhere under an extensionpoint org.eclipse.ui.views
	...
	private boolean perspectiveAlreadyExists(Document currentDoc, String perspectiveId) throws XPathExpressionException{
		XPath xpath = XPathFactory.newInstance().newXPath();
		// look for an element perspective with an ID = perspectiveId anywhere under an extensionpoint org.eclipse.ui.perspectives
		String xpathQuery = String.format("//%s[@%s='%s']/view[@id='%s']", ELEM_EXTENSION, ELEM_POINT, VALUE_EXT_POINT_PERSPECTIVES, perspectiveId);  //$NON-NLS-1$
		NodeList nlViewExtensions = (NodeList) xpath.evaluate(xpathQuery, currentDoc, XPathConstants.NODESET);
		return nlViewExtensions.getLength() > 0;
	}
	...
	private void removeOldAssemblies(){
		XPath xpath = XPathFactory.newInstance().newXPath();
		try {
			NodeList nlExt = (NodeList) xpath.evaluate(String.format("//%s[@%s='%s']", ELEM_EXTENSION, ATTR_EXTENSION_POINT, VALUE_EXT_POINT_ASSEMBLIES), doc, XPathConstants.NODESET); //$NON-NLS-1$
