...
public class RidgetGenerator {
	...
	/**
	 * Parses the basisCreatePartControl-Method for calls to addUIControl(control, ridgetId) and UIControlFactory.*.
	 * This method finds all nested calls and calls in inner classes, but does not find any calls in other Toplevel-Classes. 
	 * 
	 * @param fullyQualifiedClassName
	...
	/**
	 * Generates a configureRidget-Method for the given List of {@link SwtControl}. If the configureRidgets does not yes exist, it will be created.<p>
	 * It then generates the calls to getRidget("ridgetId") for all {@link SwtControl} that don't already exist. 
	 * 
	...
	private void generateGetRidgetCalls(AST ast, MethodDeclaration parentMethod, List<SwtControl> controls){
		...
		for (SwtControl control: controls){
			...
			frag.setName(ast.newSimpleName(control.getRidgetId()));
			...
			arg.setLiteralValue(control.getRidgetId());   
			mi.arguments().add(arg);
			
			CastExpression ce = ast.newCastExpression();
			ce.setType(ast.newSimpleType(ast.newSimpleName(control.getRidgetClassName())));
			ce.setExpression(mi);
				
			frag.setInitializer(ce);		
		}
	}
	
	/**
	 * Adds an Importstatement to the given {@link CompilationUnit} if necessary, for every Ridget-Class in the controls-List.
	 * 
	 * @param ast
	 * @param unit
	 * @param controls
